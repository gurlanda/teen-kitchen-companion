...
class EntryInputState implements Clonable<EntryInputState>, Identifiable {
  ...
  constructor(journal: Journal, inputStates?: QuestionInputState[], submittedAt?: Date, entryId?: string) {
  ...
  static fromEntry(originalJournal: Journal, entry: Entry): EntryInputState {
    if (originalJournal.id !== entry.journalId) {
      throw new Error('In EntryInputState.fromResponse(): Entry ID from Entry does not match Entry ID in Entry.')
    }
  ...
  applyQuestionReducer(targetId: string, reducer: Reducer<QuestionInputState>): EntryInputState {
