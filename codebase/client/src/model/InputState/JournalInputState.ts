...
class JournalInputState implements Clonable<JournalInputState>, Identifiable {
  ...
  get journalId(): string { return this.journal.id; }
  get weekStart(): Date { return this.journal.weekStart; }
  get weekEnd(): Date { return this.journal.weekEnd; }

  constructor(journal: Journal, inputStates?: QuestionInputState[], entryId?: string) {
  ...
  static fromEntry(originalJournal: Journal, entry: Entry): JournalInputState {
    if (originalJournal.id !== entry.journalId) {
      throw new Error('In JournalInputState.fromEntry(): Journal ID from Journal does not match Journal ID in Entry.')
    }
  ...
  applyQuestionReducer(targetId: string, reducer: Reducer<QuestionInputState>): JournalInputState {
