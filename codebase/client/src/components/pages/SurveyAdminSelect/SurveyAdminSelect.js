...
import SurveyAdminItem from './SurveyAdminItem';
import Loading from '../../layout/Loading';

const SurveyAdminSelect = () => {
  ...
  const createSurvey = async () => {
    ...
    history.push(`/survey/admin/id/${newSurvey.id}`);
  };
      ...
      <div className='sm:pt-7 mt-5 tk-acumin-pro text-gray-600 sm:w-screen-3/5 sm:mx-auto sm:shadow-md sm:shadow-slate-200 sm:rounded-lg bg-white lg:max-w-[560px]'>
        ...
        {surveys.length > 0 ? (
          <>
            ...
            {surveys.map((survey) => (
              <SurveyAdminItem key={survey.id} survey={survey} />
            ))}
          </>
        ) : (
          <h1 className='mb-7 text-md text-center'>
...
export default SurveyAdminSelect;
