...
const MultChoiceGridInput = ({ questionId }) => {
  const surveyContext = useContext(SurveyContext);
  const question = surveyContext[questionId];
  const { didAttemptSubmit } = surveyContext;
  const { text, rowQuestions, options, isOneToOne, isRequired } = question;
  ...
  const onChange = (event) => {
    ...
    surveyContext.applyQuestionReducer(
      question.createSetOption(rowQuestionId, affectedOption),
      questionId
    );
  };
